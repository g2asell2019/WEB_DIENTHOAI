...
export const PhoneCard = ({ addToCart }) => {
  ...
  return (
    <>
        ...
        arrbrand.map((gia, stt) => {
          ...
          return (
            <div
              ...
              key={stt}
              onClick={() => handleBrandClick(gia.id)}
            >
      ...
      <span
        className={`high ${orderBy === "price-desc" ? "active" : ""}`}
        onClick={() => handleOrderClick("price-desc")}
      >
        <i className="fas fa-sort-amount-up-alt mr-2"></i>Giá Cao-Thấp
      </span>
      <span
        className={`low ${orderBy === "price-asc" ? "active" : ""}`}
        onClick={() => handleOrderClick("price-asc")}
      >
